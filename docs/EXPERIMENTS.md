...
| `a1-smoke` | 2026-09-23 | `5cedefd` | A1 trainer smoke: M model, 64-frame clips, batch 4/GPU, 300 steps, FlexAttention, bf16; pilot split as train and val | pending | `mixed`, 2× pro6000, 16 CPU, 128 GB, 45 min | `runs/a1-smoke-*/` | submitted | Engineering check only: throughput, memory, checkpoint, validation, sample GIF. Not a research result. First attempt `68368` (commit `26ee8a4`) failed at import in 2 min: broken `libbz2` link in the `dd-memory` environment (see `docs/hpc/environments.md`); rerun on the rebuilt `dd-memory-gpu`. Attempt 2 `68372` (`87419c5`, `dd-memory-gpu`): 300 steps in about 6.5 min on 2 GPUs, **400 frames/s**, 62.5 GB/GPU at batch 4, loss 1.18 → 0.056, val 0.056; crashed while writing the sample (cache built on CPU, fixed in `ModelCache.from_parts`). Attempt 3 on `2e9e6b8` measures the RoPE memory fix. |
