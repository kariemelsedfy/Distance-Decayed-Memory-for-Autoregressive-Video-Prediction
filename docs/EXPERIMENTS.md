...
| `a1-smoke` | 2026-09-23 | `5cedefd` | A1 trainer smoke: M model, 64-frame clips, batch 4/GPU, 300 steps, FlexAttention, bf16; pilot split as train and val | pending | `mixed`, 2× pro6000, 16 CPU, 128 GB, 45 min | `runs/a1-smoke-*/` | submitted | Engineering check only: throughput, memory, checkpoint, validation, sample GIF. Not a research result. First attempt `68368` (commit `26ee8a4`) failed at import in 2 min: broken `libbz2` link in the `dd-memory` environment (see `docs/hpc/environments.md`); rerun on the rebuilt `dd-memory-gpu`. |
